...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 30-07-2025 a las 17:31:31
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_BuscarGraduados` (IN `p_fecha_inicio` DATE, IN `p_fecha_fin` DATE, IN `p_modalidad` VARCHAR(50), IN `p_genero` VARCHAR(10), IN `p_carrera` VARCHAR(100), IN `p_facultad` VARCHAR(100))   BEGIN
    ...
    WHERE 
        ...
        AND (p_facultad IS NULL OR p_facultad = '' OR facultad = p_facultad) AND activo != 0;
END$$
...
('mk0J0MmCGjgU94enwEsm9yqNAY0bkdvksaF8kp3E', 1, '127.0.0.1', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/138.0.0.0 Safari/537.36', '', 1753888981);
